...
You can also render the tables in Markdown in a Jupyter notebook, so they will appear similar to the first table shown at the top of this document. When you print a `Summary`, it calls a method called `table`, which can also take the `tablefmt` parameter like `diff_table` to render the table appropriately for Markdown:
