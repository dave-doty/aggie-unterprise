...
summarizes each file, as well as summarizing differences between adjacent 
(in order of date generated) files. If run with no arguments like this:
...
type  `python aggie_report -h` to see all the options.''',
    ...
    dir_files_group.add_argument('-d', '--directory', type=str,
                        help='''\
directory to search for .xlsx files generated 
by AggieEnterprise. All files are processed; 
to process only some files, list them explicitly 
after the flag -f. This option is mutually 
exclusive with -f.''')
